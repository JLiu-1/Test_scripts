...
if __name__=="__main__":
    ...
    parser.add_argument('--wave','-w',type=int,default=-1)
    ...
    for i,eb in enumerate(ebs):
        ...
        for j,datafile in enumerate(datafiles):
            ...
            comm="faz_multi -z -f -a -F -i %s -o %s.out -M REL -R %f -%d %s -T %s" % (filepath,pid,eb,args.dim," ".join(args.dims),args.tuning_target)
            if args.config!=None:
