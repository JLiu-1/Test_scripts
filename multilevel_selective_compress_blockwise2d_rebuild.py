            ...
            for beta in beta_list:
                ...
                for i in range(0,block_num_x,steplength):
                    for j in range(0,block_num_y,steplength):
                        ...
                        cur_array,cur_qs,edge_qs,cur_us,_,lsd=msc2d(cur_array,0,max_step+1,0,max_step+1,error_bound,alpha,beta,9999,args.max_step,args.anchor_rate,rate_list=None,x_preded=False,y_preded=False,\
                ...
                if psnr<=bestp and bitrate>=bestb:
                    ...
                    for i in range(0,block_num_x,steplength):
                        for j in range(0,block_num_y,steplength):
                            ...
                            cur_array,cur_qs,edge_qs,cur_us,_,lsd=msc2d(cur_array,0,max_step+1,0,max_step+1,new_error_bound,alpha,beta,9999,args.max_step,args.anchor_rate,rate_list=None,x_preded=False,y_preded=False,\
                                                    sz3_interp=args.sz_interp,multidim_level=args.multidim_level,lorenzo=-1,sample_rate=0.0,min_sampled_points=100,random_access=False,verbose=False,fix_algo=args.fix_algo,fix_algo_list=None)
                                #print(len(cur_qs[max_level]))
                                #print(len(test_qs[max_level]))
                            for level in range(max_level+1):
