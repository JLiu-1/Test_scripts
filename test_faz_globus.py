...
if __name__=="__main__":
    ...
    for j,datafile in enumerate(datafiles):
        ...
        cmppath=os.path.join(cmpfolder,datafile)+".faz"
        comm="faz -z -f -a -F -i %s -z %s -o %s.out -M REL -R %f -%d %s -T %s" % (filepath,cmppath,pid,eb,args.dim," ".join(args.dims),args.tuning_target)
        #print(comm)
