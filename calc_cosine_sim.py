...
if r_or_c: #c
    input_array=normalize(input_array,axis=0)
    output_array=np.matmul(np.transpose(input_array),input_array)
else:
    input_array=normalize(input_array,axis=1)
    output_array=np.matmul(input_array,np.transpose(input_array))
