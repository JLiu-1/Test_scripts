...
import math
import random
from multilevel_selective_compress_3d_api_rebuild import msc3d
from utils import *
                        ...
                        for j in range(0,block_num_y,steplength):
                            for k in range(0,block_num_z,steplength):
                                ...
                                if curmin<themin:
                                    themin=curmin
                                '''
                                cur_array,cur_qs,edge_qs,cur_us,_,lsd=msc3d(cur_array,0,max_step+1,0,max_step+1,0,max_step+1,new_error_bound,alpha,beta,9999,args.max_step,args.anchor_rate,rate_list=None,x_preded=False,y_preded=False,\
                                                        sz_interp=args.sz_interp,selection_criteria=args.criteria,multidim_level=args.multidim_level,lorenzo=-1,sample_rate=0.0,min_sampled_points=100,random_access=False,verbose=False,fix_algo=args.fix_algo)
                                #print(len(cur_qs[max_level]))
                                #print(len(test_qs[max_level]))
                                for level in range(max_level+1):
            ...
            for y_start in range(0,last_y,block_size):
                for z_start in range(0,last_z,block_size):
                    ...
                    msc3d(cur_array,0,max_step+1,0,max_step+1,0,max_step+1,error_bound,args.rate,args.maximum_rate,min_coeff_level,max_step,anchor_rate,\
                        ...
                        sample_rate=args.fallback_sample_ratio,min_sampled_points=10,x_preded=False,y_preded=False,z_preded=False,random_access=False,fix_algo=args.fix_algo,fix_algo_list=None)
